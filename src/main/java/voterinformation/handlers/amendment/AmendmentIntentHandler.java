...
public class AmendmentIntentHandler implements RequestHandler {

    @Override
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName("AmendmentIntent").or(intentName("AMAZON.YesIntent")));
    }

    @Override
    public Optional<Response> handle(HandlerInput input) {

        Map<String, String> amendmentMap = votingInfo.getInfo();
        Map<String, String> imageMap = votingInfo.getImages();

        String amendment = amendmentMap.get("Amendment3");
        String speechText = amendment;
        String optionalText = "";
        String imgLink = imageMap.get("Amendment3");
